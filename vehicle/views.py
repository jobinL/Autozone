...
from .models import Product, Service, Bill, ServiceEntry,Address
from .forms import ServiceForm, ServiceEntryForm, AddressForm
from django.forms import modelformset_factory 


def start(request): 
    return render(request,'vehicle/start.html') 

def start1(request): 
    products = Product.objects.all()
    return render(request,'vehicle/indexeco.html', {'products':products})
...
def ecustomer_signup_view(request):
    userForm=forms.CustomerUserForm()
    customerForm=forms.CustomerForm()
    mydict={'userForm':userForm,'customerForm':customerForm}
    if request.method=='POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            my_customer_group = Group.objects.get_or_create(name='CUSTOMER')
            my_customer_group[0].user_set.add(user)
        return HttpResponseRedirect('indexeco')
    return render(request,'vehicle/ecustomersignup.html',context=mydict) 

def product_detail(request, product_id):
    product = get_object_or_404(Product, pk=product_id)
    return render(request, 'vehicle/product_detail.html', {'product': product})
...
def view_cart(request):
    ...
    return render(request, 'vehicle/cart.html', {'cart_items': cart_items, 'total_price': total_price})
...
def checkout(request):
    ...
    return render(request, 'vehicle/checkout.html', {'cart_items': cart_items, 'total_price': total_price})
...
def address(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            address = form.save()
            # Store the address id in session to retrieve later
            request.session['address_id'] = address.id
            print(f"Address saved with ID: {address.id}")
            return redirect('review_order')
    else:
        form = AddressForm()
    return render(request, 'vehicle/address.html', {'form': form})
...
def review_order(request):
    ...
    return render(request, 'vehicle/review_order.html', {
...
def process_order(request):
    if request.method == 'POST':
        # Process the order (e.g., save order details to the database, process payment)
        # Clear the cart after successful order processing
        request.session['cart'] = {}
        return render(request, 'vehicle/order_success.html')
...
def eabout(request):
     return render(request, 'vehicle/eabout.html', {}) 

#new--
def create_service(request):
    ServiceEntryFormSet = modelformset_factory(ServiceEntry, form=ServiceEntryForm, extra=3)  # Adjust extra as needed
    if request.method == 'POST':
        form = ServiceForm(request.POST)
        formset = ServiceEntryFormSet(request.POST, queryset=ServiceEntry.objects.none())
        if form.is_valid() and formset.is_valid():
            ...
            for form in formset:
                ...
                service_entry.save()
                total_cost += service_entry.service_type.cost_per_hour * service_entry.hours_spent
            Bill.objects.create(service=service, total_cost=total_cost)
            return redirect('service_detail', service_id=service.id)
    else:
        form = ServiceForm()
        formset = ServiceEntryFormSet(queryset=ServiceEntry.objects.none())
    
    return render(request, 'create_service.html', {'form': form, 'formset': formset})

def service_detail(request, service_id):
    service = Service.objects.get(id=service_id)
    service_entries = ServiceEntry.objects.filter(service=service)
    return render(request, 'service_detail.html', {'service': service, 'service_entries': service_entries}) 
